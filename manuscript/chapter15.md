...
As a senior guy, when I'm in new projects like this, I make judgments about what people are able to do by watching how they handle System Realized items. Some folks read a book like this and only want to talk about abstract stuff. Some folks hate anything to do with abstraction and just want to start pounding some nails (or heads). I can't tell you how many project kickoff meetings I've been in where a high-level business leader comes to the team, explains some situation and opportunity; the reason for the project. The floor is open for questions. First question is senior TechWeenie Oswald's: In the sharded database cluster we keep in Europe, how is the field length change going to impact CPU load?
...
All structure is derivative. That's another way of saying that any structural model must meet the requirements of both the entire behavior model and the entire supplemental model. Even if you're just looking at a small piece, that small piece has to fit in with the rest of it.[^15-421]

[^15-421]: If you've been following along, you'll notice that we never, ever stick structure in just because we think we should. This is because when we do this, 99% of the time we never need it. Instead, we only add structure as a last resort when it's demanded by the other buckets. Even at the top level, in the Master Model, we never add entities in the Domain Model simply because we think we need them. Instead, we watch and listen as the team talks, then add the Domain Model items based on what comes up in conversation a lot. Structure is always derivative.
